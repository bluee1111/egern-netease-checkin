...
// v15: 强制刷新版本（换新文件名强制 Egern 旧实例失效，避免脚本缓存）
...
async function runSign(ctx) {
  ...
  if (!entries.length) {
    ...
    ctx.notify({ title: "网易云音乐签到失败 v15", body: result.message, sound: true, duration: 6 });
    return;
  }
  const reports = [];
  ...
  ctx.storage.setJSON(LAST_RESULT_KEY, result);
  ctx.notify({
    title: failures.length ? "网易云音乐签到部分失败 v15" : "网易云音乐签到完成 v15",
...
export default async function (ctx) {
  try {
    ...
    ctx.notify({ title: "网易云音乐签到异常 v15", body: message.slice(0, 180), sound: true, duration: 6 });
  }
}